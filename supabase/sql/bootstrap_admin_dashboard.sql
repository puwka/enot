...
BEGIN
  ...
  IF to_regclass('public.news') IS NOT NULL THEN
    EXECUTE $q$ SELECT count(*) FROM public.news WHERE deleted_at IS NULL $q$
    INTO news_count;
  END IF;

  IF to_regclass('public.articles') IS NOT NULL THEN
    EXECUTE $q$ SELECT count(*) FROM public.articles WHERE deleted_at IS NULL $q$
    INTO articles_count;
  END IF;

  IF to_regclass('public.pages') IS NOT NULL THEN
    EXECUTE $q$ SELECT count(*) FROM public.pages WHERE deleted_at IS NULL $q$
    INTO pages_count;
  END IF;
